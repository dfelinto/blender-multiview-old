...
#define BPyButton_Check(v) ((v)->ob_type == &BPyButton_Type)
...
extern PyObject *g_bpystatedict;

/* store a list of buttons we make */
extern PyObject *M_Button_List; 
...
static BPyButtonObject *newbutton( void );

/* GUI interface routines */

void exit_pydraw( SpaceScript * sc, short error );
...
static char Method_Create_doc[] =
	"(value) - Create a default BPyButtonObject object\n\n\
 (value) - The value to store in the button\n\n\
 Valid values are ints, floats, and strings";

static char Method_Button_doc[] =
	"(name, event, x, y, width, height, [tooltip]) - Create a new BPyButtonObject \
(push) button\n\n\
...
This function can be called as BPyButtonObject() or PushBPyButtonObject().";

static char Method_BeginAlign_doc[] =
	"BPyButtonObjects after this function will draw aligned (button layout only)";
...
static char Method_ColorPicker_doc[] = 
	"(event, x, y, width, height, initial, [tooltip]) - Create a new BPyButtonObject \
Color picker button\n\n\
...
PyTypeObject BPyButton_Type = {
	PyObject_HEAD_INIT( NULL ) 0,	/*ob_size */
	"BPyButtonObject",		/*tp_name */
	sizeof( BPyButtonObject ),	/*tp_basicsize */
	0,			/*tp_itemsize */
...
static void Button_dealloc( PyObject * self )
{
	BPyButtonObject *but = ( BPyButtonObject * ) self;
...
static PyObject *Button_richcmpr(PyObject *objectA, PyObject *objectB, int comparison_type)
{
	...
	if (BPyButton_Check(objectA))
		objectA = valA = Button_getattr( objectA, "val" );
	if (BPyButton_Check(objectB))
...
static BPyButtonObject *newbutton( void )
{
	BPyButtonObject *but = NULL;
	
	but = ( BPyButtonObject * ) PyObject_NEW( BPyButtonObject, &BPyButton_Type );
...
static void set_pycallback(uiBut *ubut, PyObject *callback, BPyButtonObject *but)
{
	PyObject *tuple;
	if (!callback || !PyCallable_Check(callback)) {
...
static PyObject *Method_PupBlock( PyObject * self, PyObject * args )
{
	...
	for ( i=0 ; i<len ; i++ ) {
		...
		case 3:		/*	TOGGLE	*/
			if (!PyArg_ParseTuple( pyItem, "sO!|s", &text, &BPyButton_Type, &but, &tip )) {
				Py_DECREF( pyItem );
		...
		case 5:		/*	TEX and NUM	*/
			if (!PyArg_ParseTuple( pyItem, "sO!OO|s", &text, &BPyButton_Type, &but, &pyMin, &pyMax, &tip )) {
				Py_DECREF( pyItem );
...
static PyObject *Method_Image( PyObject * self, PyObject * args )
{
	...
	py_img = ( BPyImageObject * ) pyObjImage;
	image = py_img->image;
...
PyObject *ButtonType_Init( void ) 	 
{ 	 
	PyType_Ready( &BPyButton_Type );
	return (PyObject *) &BPyButton_Type; 	 
}
