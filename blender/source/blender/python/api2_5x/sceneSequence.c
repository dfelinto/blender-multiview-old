...
/*****************************************************************************/
/* Python BPySequenceObject methods table:					   */
/*****************************************************************************/
static PyObject *Sequence_copy( BPySequenceObject * self );
static PyObject *Sequence_new( BPySequenceObject * self, PyObject * args );
static PyObject *Sequence_remove( BPySequenceObject * self, PyObject * args );

static PyObject *SceneSeq_new( BPySceneObjectSeq * self, PyObject * args );
static PyObject *SceneSeq_remove( BPySceneObjectSeq * self, PyObject * args );
static void intern_pos_update(Sequence * seq); 

static PyMethodDef BPySequence_methods[] = {
...
static PyMethodDef BPySceneSeq_methods[] = {
	/* name, method, flags, doc */
...
static PyObject *NewSeq_internal(ListBase *seqbase, PyObject * args, Scene *sce)
{
	...
	} else if (BPySound_Check(py_data)) {
		/* sound */
		int totframe;
...
{
	BPySequenceObject *bpy_seq = NULL;
	
	if( !PyArg_ParseTuple( args, "O!", &BPySequence_Type, &bpy_seq ) )
		return EXPP_ReturnPyObjError( PyExc_ValueError,
...
/*****************************************************************************/
static PyObject *Sequence_repr( BPySequenceObject * obj );
static PyObject *SceneSeq_repr( BPySceneObjectSeq * obj );

/*****************************************************************************/
/* Python BPySequenceObject methods:                                                  */
/*****************************************************************************/


static PyObject *Sequence_getIter( BPySequenceObject * self )
{
...
static PyObject *SceneSeq_getIter( BPySceneObjectSeq * self )
{
 ...
 * Return next Seq
 */
static PyObject *Sequence_nextIter( BPySequenceObject * self )
{
	PyObject *object;
	if( !(self->iter) ) {
 ...
 * Return next Seq
 */
static PyObject *SceneSeq_nextIter( BPySequenceObject * self )
{
	PyObject *object;
	if( !(self->iter) ) {
		self->iter = NULL; /* so we can add objects again */
...
static PyObject *Sequence_getName( BPySequenceObject * self )
{
	return PyString_FromString( self->seq->name+2 );
}

static int Sequence_setName( BPySequenceObject * self, PyObject * value )
{
	char *name = NULL;
...
static PyObject *Sequence_getSound( BPySequenceObject * self )
{	/* NULL is ok here -> None */
	return Sound_CreatePyObject(self->seq->sound);
}

static PyObject *Sequence_getIpo( BPySequenceObject * self )
{	/* NULL is ok here -> None */
	return Ipo_CreatePyObject( self->seq->ipo );
}

static PyObject *SceneSeq_getActive( BPySceneObjectSeq * self )
{
	...
	/* if last_seq == NULL -> None, this is ok */
	return Sequence_CreatePyObject(last_seq, NULL, self->scene );
}

static PyObject *SceneSeq_getMetaStrip( BPySceneObjectSeq * self )
{
	Sequence *seq = NULL;
	Editing *ed = self->scene->ed;
	MetaStack *ms;
...
static int Sequence_setIpo( BPySequenceObject * self, PyObject * value )
{
	Ipo *ipo = NULL;
	Ipo *oldipo;
	ID *id;
...
static PyObject *Sequence_getScene( BPySequenceObject * self )
{	/* self->seq->scene can be NULL */
	return Scene_CreatePyObject( self->seq->scene );
}


static PyObject *Sequence_getImages( BPySequenceObject * self )
{
	Strip *strip;
	StripElem *se;
	int i;
 ...
 */
static PyObject *getIntAttr( BPySequenceObject *self, void *type )
{
	int param;
...
static int setIntAttrClamp( BPySequenceObject *self, PyObject *value, void *type )
{
	struct Sequence *seq= self->seq;
...
static PyObject *getFlagAttr( BPySequenceObject *self, void *type )
{
...
static int setFlagAttr( BPySequenceObject *self, PyObject *value, void *type )
{
	int t = (int)type;
...
/*****************************************************************************/
static PyGetSetDef BPySequence_getset[] = {
	{"name",
...
/* Python attributes get/set structure:                                      */
/*****************************************************************************/
static PyGetSetDef BPySceneSeq_getset[] = {
	{"active",
...
/*****************************************************************************/
PyTypeObject BPySequence_Type = {
	PyObject_HEAD_INIT( NULL )  /* required py macro */
	0,                          /* ob_size */
	/*  For printing, in format "<module>.<name>" */
	"Blender Sequence",             /* char *tp_name; */
	sizeof( BPySequenceObject ),         /* int tp_basicsize; */
  ...
  /*** Attribute descriptor and subclassing stuff ***/
	BPySequence_methods,           /* struct PyMethodDef *tp_methods; */
	NULL,                       /* struct PyMemberDef *tp_members; */
	BPySequence_getset,         /* struct PyGetSetDef *tp_getset; */
...
PyTypeObject BPySceneSeq_Type = {
  ...
  /*** Attribute descriptor and subclassing stuff ***/
	BPySceneSeq_methods,           /* struct PyMethodDef *tp_methods; */
	NULL,                       /* struct PyMemberDef *tp_members; */
	BPySceneSeq_getset,         /* struct PyGetSetDef *tp_getset; */
	NULL,                       /* struct _typeobject *tp_base; */
	NULL,                       /* PyObject *tp_dict; */
	NULL,                       /* descrgetfunc tp_descr_get; */
	NULL,                       /* descrsetfunc tp_descr_set; */
...
{
	BPySequenceObject *pyseq;

	if( !seq )
		Py_RETURN_NONE;

	pyseq =
		( BPySequenceObject * ) PyObject_NEW( BPySequenceObject, &BPySequence_Type );
...
{
	BPySceneObjectSeq *pysceseq;

	if( !scn )
		Py_RETURN_NONE;

	pysceseq =
		( BPySceneObjectSeq * ) PyObject_NEW( BPySceneObjectSeq, &BPySceneSeq_Type );
...
/* Description: This is a callback function for the BPySequenceObject type. It	 */
/*		builds a meaninful string to represent object objects.	 */
/*****************************************************************************/
static PyObject *Sequence_repr( BPySequenceObject * self )
{
	return PyString_FromFormat( "[Sequence Strip \"%s\"]",
					self->seq->name + 2 );
}
static PyObject *SceneSeq_repr( BPySceneObjectSeq * self )
{
	return PyString_FromFormat( "[Scene Sequence \"%s\"]",
				self->scene->id.name + 2 );
}

PyObject *SequenceType_Init( void )
{
	PyType_Ready( &BPySequence_Type );
	return (PyObject *) &BPySequence_Type;
}

PyObject *SceneSeqType_Init( void )
{
	PyType_Ready( &BPySceneSeq_Type );
	return (PyObject *) &BPySceneSeq_Type;
}
