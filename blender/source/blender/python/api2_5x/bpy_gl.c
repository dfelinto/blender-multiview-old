/* 
 * $Id: BGL.c 11437 2007-07-30 20:04:23Z campbellbarton $
 *
...
#include "bpy_gl.h" /*This must come first */
...
static PyObject *GLBuffer_slice(BPyGLBufferObject *self, int begin, int end)
{
	...
	for (count= begin; count<end; count++)
		PyList_SetItem(list, count-begin, GLBuffer_item(self, count));
...
static int GLBuffer_ass_item(BPyGLBufferObject *self, int i, PyObject *v)
{
	...
	if (self->ndimensions!=1) {
		...
		if (!row) return -1;
		ret= GLBuffer_ass_slice(row, 0, self->dimensions[1], v);
		Py_DECREF(row);
		return ret;
	}

	if (self->type==GL_BYTE) {
		if (!PyArg_Parse(v, "b;Coordinates must be ints", &self->buf.asbyte[i]))
		return -1;
	} else if (self->type==GL_SHORT) {
		if (!PyArg_Parse(v, "h;Coordinates must be ints", &self->buf.asshort[i]))
			return -1;
	  
	} else if (self->type==GL_INT) {
		if (!PyArg_Parse(v, "i;Coordinates must be ints", &self->buf.asint[i]))
			return -1;
	} else if (self->type==GL_FLOAT) {
		if (!PyArg_Parse(v, "f;Coordinates must be floats", &self->buf.asfloat[i]))
			return -1;
	} else if (self->type==GL_DOUBLE) {
		if (!PyArg_Parse(v, "d;Coordinates must be floats", &self->buf.asdouble[i]))
			return -1;
	}
	return 0;
}

static int GLBuffer_ass_slice(BPyGLBufferObject *self, int begin, int end, PyObject *seq)
{
	PyObject *item;
	int count, err=0;
	...
	for (count= begin; count<end; count++) {
		...
		err= GLBuffer_ass_item(self, count, item);
		Py_DECREF(item);
		if (err) break;
	}
	return err;
...
static PyObject *GLBuffer_tolist(BPyGLBufferObject *self)
{
	...
	for (i=0; i<len; i++) {
	  PyList_SetItem(list, i, GLBuffer_item(self, i));
	}
...
static PyObject *GLBuffer_dimensions(BPyGLBufferObject *self)
{
	...
	for (i= 0; i<self->ndimensions; i++) {
	  PyList_SetItem(list, i, PyInt_FromLong(self->dimensions[i]));
	}
...
static PyObject *GLBuffer_getattr(BPyGLBufferObject *self, char *name)
{
	if (strcmp(name, "list")==0) return GLBuffer_tolist(self);
	else if (strcmp(name, "dimensions")==0) return GLBuffer_dimensions(self);
...
static PyObject *GLBuffer_repr(BPyGLBufferObject *self)
{
	PyObject *list= GLBuffer_tolist(self);
...
static struct PyMethodDef BGL_methods[] = {
  {"Buffer", Method_GLBuffer, METH_VARARGS, Method_GLBuffer_doc}, 
...
{
	PyType_Ready( &BPyGLBuffer_Type );
	return (PyObject *) &BPyGLBuffer_Type;
}

PyObject *m_gl_init(void)
{
	PyObject *mod= Py_InitModule("bpy.gl", BGL_methods);
	PyObject *dict= PyModule_GetDict(mod);
	
	if( PyType_Ready( &BPyGLBuffer_Type) < 0)
		Py_RETURN_NONE;
