	...
	for(y=pa->disprect.ymin; y<pa->disprect.ymax; y++) {
		for(x=pa->disprect.xmin; x<pa->disprect.xmax; x++, zrect++, rgbrect+=4) {
			...
			for(go=R.lights.first; go; go= go->next) {
				...
				{
					/* if it's sky check aerial perspective over sky flag */
					if(*zrect >= 9.9e10 && 
						(lar->sunsky->effect_type & LA_SUN_EFFECT_AP_OVER_SKY)==0){
						continue;
					}
					...
					if(lar->sunsky->effect_type & LA_SUN_EFFECT_AP){	
						...
						if(*zrect >=9.9e10)
							shadeAtmPixel(lar->sunsky, tmp_rgb, x, y, lar->energy, lar->sunsky->atm_DistanceOverSky);
						else
							shadeAtmPixel(lar->sunsky, tmp_rgb, x, y, lar->energy, *zrect);
						if(first_lamp){
							VECCOPY(rgb, tmp_rgb);
							first_lamp = 0;						
						}
